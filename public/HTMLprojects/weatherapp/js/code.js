...
let currentCity 
// = localStorage.getItem('currentCity') ?? 'Molde'
...
const updatePage = async (cityname) => {
    try {
        if(isUndefined(cityname)){
           cityname = localStorage.getItem('currentCity') ?? 'Molde'
        }
        myWeatherData = (await getData(cityname)).data
...
const injectHours = () => {
    ...
    const adjustedTime = parseInt((myWeatherData.days[0].hours[calculateCityTime(myWeatherData)].datetime).slice(0,2))

    for (var i = 0; i < myWeatherData.days[0].hours.length / 2; ++i) {
        let hourdata = myWeatherData.days[0].hours[Math.floor(adjustedTime + i * 2)]
