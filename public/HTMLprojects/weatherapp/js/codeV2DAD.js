...
const updatePage = async () => { // DAD you could just set currentCity first and then not send it in here
    try {
        ...
        createChart() // DAD create chart probably throws an error because there is no access to the class/module in new Chart, and arr is undefined
...
const createChart = () => {
    ...
    myWeatherData.days[0].hours.forEach((e, i) => {
        temparr.push(e.temp)
        if (adjustedTime + i < 24) { hourarr.push(adjustedTime + i) }
        else { hourarr.push(adjustedTime + i - 24) }
    });  
        ...
        chartElement,
        {
            ...
            data: {
                ...
                datasets: [{
                    data: temparr,
                    borderWidth: 1,
                    borderColor: '#1c75bc', //temparr.map(value => value <= 0 ? 'rgba(0, 0, 255, 1)' : 'rgba(255, 99, 132, 1)')
                    borderWidth: 3,
                    backgroundColor: temparr.map(value => value <= 0 ? 'rgba(0, 0, 255, 1)' : 'rgba(255, 99, 132, 1)'),
