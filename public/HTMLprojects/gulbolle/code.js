...
let teamPageDiv = document.createElement("div");
let wordInputPage = document.createElement("div");
...
const createWordInputPage = () => {
    ...
    const checkWord = () => {
        if(wordinput.value){
            const addWord = () =>{
                wordBank.push(wordinput.value)
                addtoWordList(wordinput.value)
                console.log("word added", wordinput.value);

                wordinput.value = "";
            }
            const failedWord = () => {
                console.log("word failed", wordinput.value);
                wordinput.value = "";
            }
...
const startGameMode = (mode) => {
    if (mode) {
        ...
        const displayWord = () => {
            ...
            if(word !== null){
                wordDisplay.append(document.createTextNode(word))
                gamePage.append(wordDisplay);
                currentWord = word
            }
            else{
                wordDisplay.append(document.createTextNode("OUT OF WORDS"))
